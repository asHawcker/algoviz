...
const PreorderTraversal: React.FC = () => {
  ...
  const performStep = useCallback(() => {
    ...
    if (currentNodeId !== null) {
      ...
      setStatusText(`Visiting ${node.value}, pushing to stack, moving to left child.`);
      setTraversedOutput(prev => [...prev, node.value]);

      setTraversalState({
          ...traversalState,
          visitedIds: new Set(visitedIds).add(currentNodeId),
    ...
    if (stack.length > 0) {
      ...
      setStatusText(`Popped ${poppedNode.value}. Moving to its right child.`);
  ...
  const getNodeColor = (nodeId: number) => {
    // A node is "Visited" (green) as soon as it's added to the output in preorder.
    if (isComplete || traversalState.visitedIds.has(nodeId)) return '#22c55e'; // green-500
    // It is "In Stack" (blue) after being visited but before its children are processed.
